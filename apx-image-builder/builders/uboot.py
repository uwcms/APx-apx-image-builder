...
'''.strip()
	...
	def check(self, STAGE: base.Stage, PATHS: base.BuildPaths, LOGGER: logging.Logger) -> bool:
		...
		if STAGE.name in ('fetch', 'prepare'
			...
			LOGGER.error(
			    'Please set a `uboot_tag` or `uboot_sourceurl` (file://... is valid) in the configuration for the "uboot" builder.'
			)
			check_ok = False
	...
	def nconfig(self, STAGE: base.Stage, PATHS: base.BuildPaths, LOGGER: logging.Logger) -> None:
		...
		if not (ubdir / '.config').exists():
			base.fail(LOGGER, 'No U-Boot configuration file was found.  Use uboot:defconfig to generate one.')
